...
import type { VideoChatRequest, ConversationResponse } from "@/lib/types";
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.conversation_id && !body.video_url && !body.video_id) {
      return NextResponse.json(
        { error: "Must provide either `conversation_id`, `video_url`, or `video_id`." },
        { status: 400 }
      );
    }
    ...
    if (body.video_url) backendBody.video_url = body.video_url;
    if (body.video_id) backendBody.video_id = body.video_id;

    const backendUrl = `${API_BASE}/api/v2/chat/video`;
    ...
    console.error("POST /api/v2/chat/video error:", err);
